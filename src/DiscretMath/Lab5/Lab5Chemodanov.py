...
g = [int(i) for i in "111010001"]  # порождающий многочлен
n = 15  # общее число элементов
m = 7  # число информационных элементов
length = len(g)
...
g = [int(i) for i in "1000111110101111"]  # порождающий многочлен
...
for i in range(m):
    G0.append(np.roll(g, i))
G = np.array(G0)  # порождающая матрица
codewords_table = np.array("0000000000000000000000000000000")
length = pow(2, m)
